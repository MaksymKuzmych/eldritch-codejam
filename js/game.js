...
function randomCards(difficultGreen, difficultBrown, difficultBlue) {
  const allGreenCards = difficultGreen;
  const allBrownCards = difficultBrown;
  const allBlueCards = difficultBlue;

  firstStageArr = [
    ...cardsForStages(0, allGreenCards),
    ...cardsForStages(1, allBrownCards),
    ...cardsForStages(2, allBlueCards),
  ];
  secondStageArr = [
    ...cardsForStages(3, allGreenCards),
    ...cardsForStages(4, allBrownCards),
    ...cardsForStages(5, allBlueCards),
  ];
  thirdStageArr = [
    ...cardsForStages(6, allGreenCards),
    ...cardsForStages(7, allBrownCards),
    ...cardsForStages(8, allBlueCards),
  ];
}
...
function cardsForStages(numberInRow, allColorCards) {
  ...
  while (res.length + 1 <= cardCounter[numberInRow].textContent) {
    const randomIndex = Math.floor(Math.random() * allColorCards.length);
...
function revealCards(stageArr, cards) {
  const randomIndex = Math.floor(Math.random() * stageArr.length);
  const deleted = stageArr.splice(randomIndex, 1)[0];
...
gameBtn.addEventListener('click', () => {
  const chosenRules = document.querySelectorAll('.chosen');
  ...
  const ancientObj = ancientsData.filter((el) => el.id == ancient);
  ...
  if (difficulty.toLowerCase() === 'easy') {
    randomCards(
      [...greenEasyCards, ...greenNormalCards],
      [...brownEasyCards, ...brownNormalCards],
      [...blueEasyCards, ...blueNormalCards]
    );
  } else if (difficulty.toLowerCase() === 'medium') {
    randomCards(greenCards, brownCards, blueCards);
  } else if (difficulty.toLowerCase() === 'hard') {
    randomCards(
      [...greenNormalCards, ...greenHardCards],
      [...brownNormalCards, ...brownHardCards],
      [...blueNormalCards, ...blueHardCards]
    );
  }
});
